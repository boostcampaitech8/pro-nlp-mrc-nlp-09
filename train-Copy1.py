...
def run_mrc(
    ...
    if training_args.do_train:
        ...
        train_dataset = datasets["train"]
                # 🔥 Negative Sampling 적용 (optional)
        if getattr(data_args, "use_negative_sampling", False):
            from negative_sampling import NegativeSampler
            from src.retrieval import SparseRetrieval  # BM25 retriever
    
            print("🔄 Negative sampling enabled — generating augmented train dataset")
    
            # 1) 기존 training corpus 로딩
            retriever = SparseRetrieval(
                tokenize_fn=lambda x: tokenizer.tokenize(x),
                data_path="./data",                      # 디렉토리
                context_path="wikipedia_documents.json"  # 실제 파일명
            )
            retriever.get_sparse_embedding()  # BM25 준비
    
            # 2) NegativeSampler 생성
            ...
            print(f"✅ Negative sampling applied: Train size → {len(train_dataset)}")
